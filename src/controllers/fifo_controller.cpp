...
void FIFOController::openNamedPipes() {
  m_run_id = m_osystem->settings().getInt("run_id"
    );
    char fifo_out[32];
    snprintf(fifo_out, sizeof(char) * 32, "ale_fifo_out_%i", m_run_id);


  m_fout = fopen(fifo_out, "w");
  ...
  if (m_fout == NULL) {
    ...
    char fifo_in[32];
    snprintf(fifo_in, sizeof(char) * 32, "ale_fifo_in_%i", m_run_id);
  m_fin = fopen(fifo_in, "r");
